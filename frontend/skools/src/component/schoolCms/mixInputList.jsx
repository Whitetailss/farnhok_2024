...
const PureDisplay = ({ teacherInputArr }) => {
  const renderInput = (mixInput, index) => {
    return (
      <div className='teacherCard' key={index}>
        <div className='flexRow'>
          <div>
            <img className='teacherImg' src={mixInput.imgUrl} alt='caption' />
          </div>
          <div className='textBox'>{mixInput.caption}</div>
        </div>
      </div>
  ...
  const displayRow = teacherInputArr.map((mixInput, index) =>
    renderInput(mixInput, index)
  );

  return (
    <div>
      <div className='scrollWrapper'>{displayRow}</div>
    </div>
  );
};

const PureDisplayTwo = ({ facilityInputArr }) => {
  const renderInput = (mixInput, index) => {
    return (
      <div className='facilityCard' key={index}>
        <div>
  ...
  const displayRow = facilityInputArr.map((mixInput, index) =>
    renderInput(mixInput, index)
  );
