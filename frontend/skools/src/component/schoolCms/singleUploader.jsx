...
const PureUploader = ({ style, text, hasCaption, hasClear, uploaderName, uploadMedia }) => {
  ...
  const handleUpload = async (e) => {
    ...
    if ((uploaderName === 'teacher' || uploaderName === 'facility') && caption === '') {
      setErrMsg('Do not leave caption/description blank');
    } else {
      let uploadTask = storage.ref(`image/${image.name}`).put(image);
  ...
  return (
    <div style={style}>
      ...
      <div className='uploadBar'>
        ...
        <div>
          <Button name={uploaderName} onClick={handleUpload}>
            Upload
          </Button>
          <br />
        </div>
      ...
      <div className='progressBar'>
        <div>
          <Progress bar value={progress} max='100'>
            LOADING
          </Progress>
          <br />
        </div>
        <div>{errMsg}</div>
        <div>
          <Button className={hasClear} color='primary' onClick={clearInput}>
            Add Another
          </Button>
        </div>
      </div>
